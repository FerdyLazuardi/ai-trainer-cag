...
def _get_system_memory() -> dict:
    """Read Linux /proc/meminfo on Proxmox VM with safe fallback."""
    try:
        ...
        pass
    return {"total_mb": 4096.0, "used_mb": 1420.0, "percent": 34.7}


@router.get("/telemetry", summary="Get Proxmox VM system telemetry")
async def get_system_telemetry(_=Depends(verify_api_key)) -> Dict[str, Any]:
    from app.database.redis_client import get_redis_client
    from app.database.postgres import engine
    ...
    try:
        ...
        redis_mem = str(info.get("used_memory_human", "—"))
    except Exception as e:
